...
namespace RayZath
{
	namespace CudaEngine
	{
			...
			cudaStream_t& mirror_stream)
		{
			...
			// material
			if (hSphere->GetMaterial().GetResource()->GetId() < hCudaWorld.materials.GetCount())
			{
				this->material = hCudaWorld.materials.GetStorageAddress() + hSphere->GetMaterial().GetResource()->GetId();
			}
			else
			{
