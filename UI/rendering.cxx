...
namespace RayZath::UI
{
    ...
    Rendering::Rendering()
    {
        ...
        VkResult err = glfwCreateWindowSurface(g_Instance, mp_glfw_window , mp_vk_allocator, &surface);
        check_vk_result(err);
        ...
        while (!glfwWindowShouldClose(mp_glfw_window ))
        {
            ...
            if (g_SwapChainRebuild)
            {
                ...
                if (width > 0 && height > 0)
                {
                    ...
                    ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, &m_imgui_main_window, g_QueueFamily, mp_vk_allocator, width, height, g_MinImageCount);
    ...
    void Rendering::SetupVulkan(const char** extensions, uint32_t extensions_count)
    {
        ...
        // Create Vulkan Instance
        {
            ...
            // Create Vulkan Instance
            err = vkCreateInstance(&create_info, mp_vk_allocator, &g_Instance);
            check_vk_result(err);
            free(extensions_ext);
            ...
            err = vkCreateDebugReportCallbackEXT(g_Instance, &debug_report_ci, mp_vk_allocator, &g_DebugReport);
            check_vk_result(err);
            #else
        ...
        // Create Logical Device (with 1 queue)
        {
            ...
            err = vkCreateDevice(g_PhysicalDevice, &create_info, mp_vk_allocator, &g_Device);
            check_vk_result(err);
        ...
        // Create Descriptor Pool
        {
            ...
            err = vkCreateDescriptorPool(g_Device, &pool_info, mp_vk_allocator, &g_DescriptorPool);
            check_vk_result(err);
        }
    }
    void Rendering::SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
    {
        m_imgui_main_window.Surface = surface;

        // Check for WSI support
        VkBool32 res;
        vkGetPhysicalDeviceSurfaceSupportKHR(g_PhysicalDevice, g_QueueFamily, m_imgui_main_window.Surface, &res);
        if (res != VK_TRUE)
        {
        ...
        m_imgui_main_window.SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(g_PhysicalDevice, m_imgui_main_window.Surface, requestSurfaceImageFormat, (size_t)IM_ARRAYSIZE(requestSurfaceImageFormat), requestSurfaceColorSpace);
        ...
        #endif
        m_imgui_main_window.PresentMode = ImGui_ImplVulkanH_SelectPresentMode(g_PhysicalDevice, m_imgui_main_window.Surface, &present_modes[0], IM_ARRAYSIZE(present_modes));

        // Create SwapChain, RenderPass, Framebuffer, etc.
        IM_ASSERT(g_MinImageCount >= 2);
        ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, wd, g_QueueFamily, mp_vk_allocator, width, height, g_MinImageCount);
    }
    void Rendering::CleanupVulkan()
    {
        vkDestroyDescriptorPool(g_Device, g_DescriptorPool, mp_vk_allocator);

        vkDestroyDevice(g_Device, mp_vk_allocator);
        vkDestroyInstance(g_Instance, mp_vk_allocator);
    }
    void Rendering::CleanupVulkanWindow()
    {
        ImGui_ImplVulkanH_DestroyWindow(g_Instance, g_Device, &m_imgui_main_window, mp_vk_allocator);
    }
    ...
    void Rendering::FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
    {
        ...
        err = vkAcquireNextImageKHR(g_Device, m_imgui_main_window.Swapchain, UINT64_MAX, image_acquired_semaphore, VK_NULL_HANDLE, &m_imgui_main_window.FrameIndex);
